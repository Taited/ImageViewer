...
def concat_page_data(data_dict: dict, category_list: list, 
                     id_begin: int = 1, fetch_num: int = 10,
                     api_path: str = '/image/image_bed'):
    assert id_begin >= 0, "The begin id could not less than 1 but get: {}".format(id_begin)
    page_data = []
    for id_name in range(id_begin, id_begin + fetch_num + 1):
        ...
        for category_name in category_list:
            if category_name == 'text_info' or category_name == 'txt':
                txt_flag = True
                data_api = osp.join(api_path, data_dict[category_name][id_name])
